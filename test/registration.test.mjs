// Tool-surface invariants for @three-ws/activity-mcp.
//
...
//
// Run: node --test packages/activity-mcp/test/registration.test.mjs
...
test('every tool is a read-only, live-data query', () => {
	for (const tool of TOOLS) {
		...
		// Live discovery data is never idempotent — rankings, the board, and the
		// feed all move between calls.
